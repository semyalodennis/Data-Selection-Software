...
/*int main()
{
	cv::String filename = "D:/CNU/C++/Opencv/GDAL2CV2/GDAL2CV2/sample_1.bil";
	Mat ImgReadByGDAL(filename, true);
...
bool KGDAL2CV::CheckDataType(const GDALDataType& gdalDataType, cv::Mat img)
{
	...
	int TypeMap_GDAL2_0[GDT_TypeCount] = { CV_8U, CV_8S , CV_16U, CV_16S, CV_32S, CV_32F, CV_64F };
...
cv::Mat KGDAL2CV::ImgReadByGDAL(cv::String filename, int xStart, int yStart, int xWidth, int yWidth, bool beReadFourth)
{
	...
	{
		for (unsigned int index = CV_8S; index < CV_64F; ++index)
		{
